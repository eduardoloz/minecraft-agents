...
def handle(cmd: dict):
    ...
    if action == 'task':
        ...
        # Interrupt the running task by closing the mineflayer connection.
        # This causes the active requests.post() inside the task thread to raise
        # a ConnectionError, which _run() catches and emits as an error event.
        if _current_thread and _current_thread.is_alive():
            try:
                odyssey.env.close()
            except Exception:
