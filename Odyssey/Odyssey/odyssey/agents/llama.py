...
# Load the shared .env so GEMINI_API_KEY is available without it ever
# touching config.json or being committed to the repo.
load_dotenv(Path(__file__).parents[3] / 'LLM-Backend' / '.env')

# All model name constants are kept so existing code that imports ModelType
# continues to work without changes. Every call routes to Gemini regardless.
class ModelType:
